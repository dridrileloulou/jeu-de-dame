...
const API = `http://127.0.0.1:${process.env.PORT || 3001}/api/dames`;
const TARGET_URL = `${process.env.FRONTEND_URL || 'http://localhost:3000'}/jeu?mode=ia&level=moyen`;
...
async function run() {
    ...
    setInterval(async () => {
        ...
        try {
            ...
            isWaitingForAi = true;
            console.log('\n🤖 ======================================');
            console.log('🤖 Tour de l\'IA (Noirs) détecté ! NIVEAU 2 (Intermédiaire)');
            
            // Vérifier s'il y a des captures obligatoires
            const mandatoryPieces = await page.evaluate(() => {
                return Array.from(document.querySelectorAll('.piece.black.mandatoryCapture'));
            });

            // S'il n'y a qu'un seul pion qui peut capturer (ou rafle en cours), on le joue automatiquement sans API
            if (mandatoryPieces.length === 1) {
                console.log(`⚡ 1 seul pion peut capturer ! Mouvement automatique...`);
                
                await page.evaluate(async () => {
                    const piece = document.querySelector('.piece.black.mandatoryCapture');
                    if (!piece) return;
                    ...
                    for (let jump of possibleJumps) {
                        const targetR = r + jump.dr;
                        const targetC = c + jump.dc;
                        const overR = r + jump.dr / 2;
                        const overC = c + jump.dc / 2;
                        
                        if (targetR >= 0 && targetR < 10 && targetC >= 0 && targetC < 10) {
                            const targetCell = boardRows[targetR].querySelectorAll('.cell')[targetC];
                            const overCell = boardRows[overR].querySelectorAll('.cell')[overC];
                            
                            if (!targetCell.querySelector('.piece') && overCell.querySelector('.piece.white')) {
                                targetCell.click();
                                break;
                            }
                        }
            ...
            const hasMandatoryCapture = mandatoryPieces.length > 0;
            if (hasMandatoryCapture) {
                console.log(`⚡ Plusieurs captures obligatoires détectées ! Demande au Minimax de choisir la meilleure...`);
            }
            ...
            const resp = await $fetch(API, { 
                method: 'POST', 
                body: { board: matrix, level: 2, player: 1, useMinimaxOnly: hasMandatoryCapture } 
            });
