...
const TARGET_URL = `${process.env.FRONTEND_URL || 'http://localhost:3000'}/jeu?mode=ia&level=facile`;
...
async function run() {
    ...
    setInterval(async () => {
        ...
        try {
            ...
            isWaitingForAi = true;
            console.log('\n🤖 ======================================');
            console.log('🤖 Tour de l\'IA (Noirs) détecté ! NIVEAU 1 (Débutant)');
            ...
            const resp = await $fetch(API, {
                method: 'POST',
                body: { board: matrix, level: 1, player: 1, useMinimaxOnly: hasMandatoryCapture }
            });
